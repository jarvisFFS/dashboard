...
function SortableItem({ task, project, onClick }) {
  ...
  // Generera en färg baserat på projekt-ID (enkel hash)
  const getProjectColor = (pid) => {
      if (!pid) return '#ccc';
      // Hårdkodade favoriter
      ...
      const hues = [200, 150, 300, 60, 25]; // Blå, Grön, Lila, Gul, Orange
      const hue = hues[hash % hues.length];
      return `hsl(${hue}, 70%, 80%)`; // Pastell
  };
...
function App() {
  ...
  const [projects, setProjects] = useState([]); // Hämta från DB
  const [activeId, setActiveId] = useState(null);
  const [editingTask, setEditingTask] = useState(null);
  const [filterProjectId, setFilterProjectId] = useState('all'); // Filter state
  ...
  const saveTaskToDb = async (task) => {
    ...
    if (error) {
        ...
        fetchData(); // Ladda om allt för att vara säker
        setEditingTask(null);
    }
  };
  ...
  const handleDragEnd = (event) => {
    ...
    // Inkludera 'archive' i listan över kolumner
  ...
  const addNewTask = (status) => {
      setEditingTask({
          id: 'new',
          title: '',
          projectId: projects[0]?.id || 'p4', // Default till första projektet
          status: status,
  ...
  // Filtrera tasks baserat på vald vy
...
export default App;/ /   V e r c e l   w a k e   u p   c a l l  
 